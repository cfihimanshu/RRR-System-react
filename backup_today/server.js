...
app.get('/api/dashboard/stats', require('./middleware/auth').verifyToken, async (req, res) => {
  try {
    ...
    if (req.user?.role?.toLowerCase().trim() === 'operation head') {
      // Operation Head: only see Odoo-sourced cases
      ownershipQuery = { sourceOfComplaint: { $regex: /^\s*odoo\s*$/i } };
    ...
    const isOperationHeadRole = req.user?.role?.toLowerCase().trim() === 'operation head';
    const isAdminOrgWideView = req.user.role === 'Admin' && !userFilter && !isLegalDashboard;
    const shouldExcludeOdooSource = !isOperationHeadRole && isAdminOrgWideView;
    ...
    const [
      ...
      track('caseMetricsFacet', () => Case.aggregate([
        { $match: query },
        {
          $facet: {
              basic: [
              ...(shouldExcludeOdooSource ? [{ $match: { sourceOfComplaint: { $not: /^\s*odoo\s*$/i } } }] : []),
              {
                $group: {
                  _id: null,
                  totalCases: { $sum: 1 },
