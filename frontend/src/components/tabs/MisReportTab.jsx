...
const MisReportTab = () => {
  ...
  const fetchData = async (silent = false) => {
    try {
      ...
      const res = await api.get('/reports/mis');
      setData(res.data);
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateTarget = async (userId, currentTarget) => {
    if (editingTargetId === userId) {
      // Save
      ...
      try {
        ...
        await api.put(`/users/${userId}/target`, { monthlyTarget: numVal });
      ...
      setNewTargetValue(currentTarget.toString());
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      const perfData = data.assigneePerformance.map(p => ({
  ...
  const filteredTodayCases = useMemo(() => {
    if (!data) return [];
    return data.todayCases.filter(c => 
  ...
  const currentSpecialist = data.assigneePerformance[activeTabIdx] || null;
      ...
      <div className="p-6 md:p-8 space-y-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-bg-card border-2 border-border rounded-2xl p-6 relative overflow-hidden group shadow-sm hover:shadow-md transition-all">
            ...
            <div className="text-[10px] text-text-muted font-bold uppercase tracking-wide mt-1">Across all specialists</div>
          </div>
          ...
          <div className="bg-bg-card border-2 border-border rounded-2xl p-6 relative overflow-hidden group shadow-sm hover:shadow-md transition-all">
            ...
            <div className="text-[10px] text-text-muted font-bold uppercase tracking-wide mt-2">Newly Registered Today</div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 border-b border-border">
            {data.assigneePerformance.map((spec, idx) => (
              <button
                key={idx}
                onClick={() => {
                  setActiveTabIdx(idx);
              ...
              <div className="bg-bg-secondary/40 border border-border rounded-2xl p-6 space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pt-2">
                  <div className="flex items-center gap-3">
                    ...
                    {editingTargetId === currentSpecialist.userId ? (
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                        ...
                        />
                        <button
                          onClick={() => handleUpdateTarget(currentSpecialist.userId, currentSpecialist.target)}
                          disabled={submittingTarget}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-bg-card border-2 border-border rounded-2xl p-6 flex flex-col min-h-[400px]">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-4 bg-accent rounded-full" />
                <h3 className="text-sm font-black text-text-primary uppercase tracking-wider">Active Cases Ledger</h3>
              </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {filteredActiveCases.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="text-center py-12 text-text-muted font-black uppercase tracking-widest text-[10px] opacity-40">No active cases found</td>
                    </tr>
                  ) : (
                    filteredActiveCases.map((c, idx) => {
                      ...
                      return (
                        <tr key={idx} className="border-b border-border hover:bg-bg-card-hover transition-colors">
                          ...
                          <td className="px-4 py-3.5 text-xs font-bold text-text-primary max-w-[150px] truncate">{c.companyName}</td>
