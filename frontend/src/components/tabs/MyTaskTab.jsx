...
const MyTaskTab = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-bg-primary">
            ...
            <div key={col.id} className="flex flex-col h-full min-w-0" onDragOver={onDragOver} onDrop={(e) => onDrop(e, col.id)}>
              ...
              >
                <div className="flex items-center gap-2">
                  <div className={`p-2 rounded-lg ${col.id === 'To Do' ? 'bg-blue-soft text-blue-400' : col.id === 'In Progress' ? 'bg-accent-soft text-accent' : 'bg-green-soft text-green-400'} border border-border`}>{col.icon}</div>
                  <h2 className="font-black text-text-primary uppercase tracking-widest text-xs">{col.label}</h2>
                  <span className="bg-bg-input text-text-muted text-[10px] font-black px-2 py-0.5 rounded-full">{filteredTasks.filter(t => t.status === col.id).length}</span>
                </div>
                <div className="md:hidden">
                  <ChevronDown size={18} className={`text-text-muted transition-transform duration-300 ${collapsedColumns[col.id] ? 'rotate-[-90deg]' : 'rotate-0'}`} />
                </div>
              </div>
              ...
              <div className={`flex-1 bg-bg-card/50 rounded-3xl border-2 border-border p-4 overflow-y-auto md:hide-scrollbar space-y-4 border-dashed min-h-[300px] ${collapsedColumns[col.id] ? 'hidden md:block' : 'block animate-in slide-in-from-top-2 duration-300'}`}>
                {filteredTasks.filter(t => t.status === col.id).length === 0 ? (
                  ...
                  filteredTasks.filter(t => t.status === col.id).map(task => (
                    ...
                    >
                      <div className={`absolute left-0 top-0 bottom-0 w-1 ${task.priority === 'High' ? 'bg-red' : task.priority === 'Medium' ? 'bg-yellow' : 'bg-blue'}`} />
                      <div className="flex justify-between items-start mb-3">
                        <span className="text-[9px] font-black text-text-muted uppercase tracking-tighter bg-bg-input px-2 py-0.5 rounded border border-border group-hover:text-accent transition-colors">
                          {task.taskId || 'TSK-NEW'}
                        </span>
                        <div className="flex gap-1 items-center">
                          <span className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest ${task.priority === 'High' ? 'bg-red-soft text-red border border-red-soft' : task.priority === 'Medium' ? 'bg-yellow-soft text-yellow border border-yellow-soft' : 'bg-blue-soft text-blue border border-blue-soft'}`}>
                            {task.priority}
                          </span>
      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-[450px] bg-bg-card shadow-2xl z-[300] transform transition-transform duration-300 ease-in-out border-l border-border flex flex-col ${isSidePanelOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {selectedTask && (
          <>
            {/* Side Panel Header */}
              ...
              <div className="flex gap-2">
                {['To Do', 'In Progress', 'Completed'].map(s => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all border ${selectedTask.status === s ? 'bg-accent border-accent text-white shadow-lg' : 'bg-bg-input border-border text-text-muted hover:text-text-primary'}`}
                  >
                    {s}
                  </button>
                ))}
            ...
            {/* Side Panel Content */}
            <div className="flex-1 overflow-y-auto p-8 space-y-10 bg-bg-card">
              {/* Core Details Table */}
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="flex justify-between items-center p-3.5 bg-bg-input rounded-2xl border border-border">
                    <span className="text-text-muted font-bold text-xs">Priority</span>
                    <span className={`font-black uppercase text-[10px] px-3 py-1 rounded-full ${selectedTask.priority === 'High' ? 'bg-red-soft text-red' : 'bg-blue-soft text-blue'}`}>{selectedTask.priority}</span>
                  </div>
                  <div className="flex justify-between items-center p-3.5 bg-bg-input rounded-2xl border border-border">
                    <span className="text-text-muted font-bold text-xs">Assigned To</span>
                    <span className="font-bold text-text-primary flex items-center gap-2 text-sm"><User size={14} className="text-accent" /> {selectedTask.assignee}</span>
                  </div>
                  <div className="flex justify-between items-center p-3.5 bg-bg-input rounded-2xl border border-border">
                  ...
                  <div className="flex justify-between items-center p-3.5 bg-bg-input rounded-2xl border border-border">
                    ...
                    <span className="font-bold text-text-primary text-[10px] flex items-center gap-1.5 uppercase tracking-wide px-3 py-1 bg-bg-card rounded-lg border border-border">
                      {selectedTask.source === 'Manual' ? <Plus size={12} className="text-green" /> : <Zap size={12} className="text-accent" />}
                      {selectedTask.source}
                    </span>
                  </div>
                </div>
            ...
            <div className="p-8 bg-bg-card border-t border-border flex gap-4">
              ...
              <button
                ...
                className="p-4 bg-red-soft text-red rounded-2xl hover:bg-red hover:text-white transition-all border border-red-soft"
                title="Delete Task"
              >
                <Trash2 size={22} />
              </button>
      ...
      >
        <form onSubmit={handleCreateTask} className="p-8 bg-bg-card">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-[11px] font-black text-text-muted uppercase tracking-widest mb-2">Select Priority</label>
              <select
                className="w-full bg-bg-input border-2 border-border rounded-2xl px-5 py-3 text-sm font-bold text-text-primary outline-none focus:border-accent focus:ring-4 focus:ring-accent-soft transition-all cursor-pointer"
            ...
            <div>
              <label className="block text-[11px] font-black text-text-muted uppercase tracking-widest mb-2">Target Date</label>
              <input
                type="date"
                className="w-full bg-bg-input border-2 border-border rounded-2xl px-5 py-3 text-sm font-bold text-text-primary outline-none focus:border-accent focus:ring-4 focus:ring-accent-soft transition-all"
            ...
            <div>
              <label className="block text-[11px] font-black text-text-muted uppercase tracking-widest mb-2">Reminder Date & Time</label>
              <input
                type="datetime-local"
                className="w-full bg-bg-input border-2 border-border rounded-2xl px-5 py-3 text-sm font-bold text-text-primary outline-none focus:border-accent focus:ring-4 focus:ring-accent-soft transition-all"
            ...
            <div>
              <label className="block text-[11px] font-black text-text-muted uppercase tracking-widest mb-2">Initial Status</label>
              <select
                className="w-full bg-bg-input border-2 border-border rounded-2xl px-5 py-3 text-sm font-bold text-text-primary outline-none focus:border-accent focus:ring-4 focus:ring-accent-soft transition-all cursor-pointer"
          ...
          <div className="mt-10 flex justify-end gap-4">
            <button
              ...
              className="px-10 py-3.5 rounded-2xl text-sm font-bold text-text-muted hover:bg-bg-input transition-all"
            >
              Cancel
            </button>
            <button
